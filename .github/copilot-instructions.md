...
- **Data flow example:** `/api/teacher/classrooms/${classroomId}/assignments/manual/route.ts` parses+validates request → calls `core/assignments/service.createScheduledAssignment` → uses `data/assignments.repo` (Prisma) → persists to DB. Preserve this end-to-end shape when modifying behavior.
